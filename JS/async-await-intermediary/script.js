...
searchBtn.addEventListener(`click`, e =>{
    ...
    apiCalls(searchBar.value)
} )

async function apiCalls(params, index=1) {
    API_KEY = `jhKRwHw5tkdNvQrIMdqoQBadySgaPB3I`
    const fetched = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=${API_KEY}&limit=20&q=${params}`);
    const fetchedData = await fetched.json()
    console.log(fetchedData)
    img.src = fetchedData.data[0].images.downsized.url
